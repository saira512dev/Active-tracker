    ...
    <div class="p-8 flex items-start bg-light-grey rounded-md shadow-lg">
      <form @submit.prevent="createWorkout" class="flex flex-col gap-y-5 w-full">
        ...
        <div v-if="workoutType === 'strength'" class="flex flex-col gap-y-4">
          ...
          v-for="(item,index) in exercises" :key="index">
           <div class="flex flex-col md:w-1/3">
            <label for="exrecise-name" class="mb-1 text-sm text-at-light-green">Exercise</label>
          ...
          <img @click="deleteExercise(item.id)" src="@/assets/images/trash-light-green.png"  class="h-4 w-auto absolute -left-5 cursor-pointer" alt="trash-can">
          </div>
           <button @click="addExercise" type="button" class="mt-6 px-6 rounded-sm self-start  text-sm text-white bg-at-light-green
        ...
        <div v-if="workoutType === 'cardio'" class="flex flex-col gap-y-4">
         ...
         <button type="submit" class="mt-6 px-6 rounded-sm self-start  text-sm text-white bg-at-light-green
              duration-200 border-solid border-2 border-transparent hover:border-at-light-green hover:bg-white
              hover:text-at-light-green">
  ...
  setup() {
    ...
    const createWorkout = async() => {
      try{
        ...
        statusMsg.value = "Success: Workout Added!"
